...
export class BrokerSample implements IBrokerWithoutRealtime {
	...
	public placeOrder(preOrder: PreOrder): Promise<void> {
	...
	public closePosition(positionId: string): Promise<void> {
	...
	public reversePosition(positionId: string): Promise<void> {
		const position = this._positionById[positionId];
		const handler = () => {
			return this.placeOrder({
				symbol: position.symbol,
